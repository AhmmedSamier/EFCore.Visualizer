...
import { MysqlPlanService } from "@visualizer/services/mysql-plan-service";
import { NodeProp, WorkerProp } from "@visualizer/enums";
import type { IPlanContent } from "@visualizer/interfaces";
      ...
      if (emptyLineMatches || headerMatches) {
        ...
        if (0 === elementsAtDepth.length) {
          ...
          if (!root.Plan) {
            ...
            if (!root.Plan[NodeProp.PLANS]) {
              root.Plan[NodeProp.PLANS] = [];
            }
            root.Plan[NodeProp.PLANS].push(newNode);
          }
          return;
        }
