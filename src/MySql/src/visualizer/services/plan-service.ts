...
import { EstimateDirection, NodeProp, WorkerProp } from "@visualizer/enums"
import type { IPlan, IPlanContent, IPlanStats } from "@visualizer/interfaces"
import { Node, Worker } from "@visualizer/interfaces"
...
export class PlanService {
      ...
      _.each(
        ...
        (node) => {
          _.each(node, (value) => {
            ...
            if (matches) {
              const exclusive = Number(node[NodeProp.EXCLUSIVE_DURATION] || 0)
              node[NodeProp.EXCLUSIVE_DURATION] = Math.max(
                0,
                exclusive - (subPlan[NodeProp.ACTUAL_TOTAL_TIME] || 0),
              )
  ...
  public async fromSourceAsync(source: string): Promise<IPlanContent> {
    if (typeof Worker !== "undefined") {
      try {
        const module = await import("@visualizer/workers/parser.worker?worker&inline")
