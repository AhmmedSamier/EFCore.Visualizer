...
import PlanNodeDetail from "@visualizer/components/PlanNodeDetail.vue"
import NodeBadges from "@visualizer/components/NodeBadges.vue"
import type { IPlan, Node, ViewOptions } from "@visualizer/interfaces"
import {
...
import { keysToString, sortKeys } from "@visualizer/filters"
import { HighlightType, NodeProp } from "@visualizer/enums"
import { findNodeBySubplanName, HelpService } from "@visualizer/services/help-service"
...
watch(showDetails, () => {
  // Wait for Vue to update the DOM then wait a bit more for layout
  nextTick(() => {
    window.setTimeout(() => updateSize(node), 100)
  })
})
...
<template>
  <div ref="outerEl" @mousedown.stop>
    <div :class="[
      ...
      <div class="plan-node-body card" :class="{
        'never-executed': isNeverExecuted,
      }" @mouseenter="highlightedNodeId = node.nodeId" @mouseleave="highlightedNodeId = undefined">
        <div class="card-body header no-focus-outline">
          <header class="mb-0 d-flex justify-content-between">
            <h4 class="text-body overflow-hidden btn btn-light text-start py-0 px-1"
              @click.prevent.stop="showDetails = !showDetails">
              <span class="text-secondary">
                <FontAwesomeIcon fixed-width :icon="faChevronUp" v-if="showDetails"></FontAwesomeIcon>
            ...
            <div class="text-nowrap">
              ...
              <a class="fw-normal small ms-1" href="" @click.prevent.stop="selectNode(node.nodeId, true)">
                #{{ node.nodeId }}
              </a>
            </div>
          </header>
          ...
          " class="node-description mb-2">
            ...
            <span class="text-muted small" v-html="getNodeTypeDescription(node[NodeProp.NODE_TYPE])"></span>
          </div>
