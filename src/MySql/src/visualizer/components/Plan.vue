...
import type { Node } from "@visualizer/interfaces"
import {
  ...
  ViewOptionsKey,
  ToggleDetailsKey,
} from "@visualizer/symbols"
import Copy from "@visualizer/components/Copy.vue"
import Diagram from "@visualizer/components/Diagram.vue"

import LogoImage from "@visualizer/components/LogoImage.vue"
import PlanNode from "@visualizer/components/PlanNode.vue"
import PlanStats from "@visualizer/components/PlanStats.vue"
import Stats from "@visualizer/components/Stats.vue"
import AnimatedEdge from "@visualizer/components/AnimatedEdge.vue"
import KeyboardShortcuts from "@visualizer/components/KeyboardShortcuts.vue"
import { HighlightType, NodeProp, Orientation } from "@visualizer/enums"
import { json_, mysql_ } from "@visualizer/filters"
import { setDefaultProps } from "vue-tippy"
import { createStore } from "@visualizer/store.ts"
import { StoreKey } from "@visualizer/symbols"
...
import { compressPlanToUrl, copyToClipboard } from "@visualizer/services/share-service"

import { usePlanLayout } from "@visualizer/composables/usePlanLayout"
...
async function parseAndShow() {
  ...
  const savedOptions = localStorage.getItem("viewOptions_v2")
  if (savedOptions) {
...
function onViewOptionsChanged() {
  localStorage.setItem("viewOptions_v2", JSON.stringify(viewOptions))
}
    ...
    <Teleport to="#header-tabs">
      <ul class="nav nav-tabs mysql-tabs flex-nowrap border-0">
        <li class="nav-item p-1">
          <a class="nav-link px-2 py-0" :class="{ active: activeTab === 'plan' }" href="javascript:void(0)"
            @click.prevent="setActiveTab('plan')">Plan</a>
        </li>

        <li class="nav-item p-1" v-if="store.plan?.content.Diagram">
          <a class="nav-link px-2 py-0" :class="{ active: activeTab === 'diagram' }" href="javascript:void(0)"
            @click.prevent="setActiveTab('diagram')">Diagram</a>
        </li>
        <li class="nav-item p-1">
          <a class="nav-link px-2 py-0" :class="{ active: activeTab === 'raw' }" href="javascript:void(0)"
            @click.prevent="setActiveTab('raw')">Raw</a>
        </li>
        <li class="nav-item p-1">
          <a class="nav-link px-2 py-0" :class="{ active: activeTab === 'query' }" href="javascript:void(0)"
            @click.prevent="setActiveTab('query')">Query</a>
        </li>
        <li class="nav-item p-1">
          <a class="nav-link px-2 py-0" :class="{ active: activeTab === 'stats' }" href="javascript:void(0)"
            @click.prevent="setActiveTab('stats')">Stats</a>
        </li>
      </ul>
    </Teleport>
            ...
            <div class="flex-grow-1 overflow-hidden">
              <Splitpanes class="default-theme" @resize="viewOptions.diagramWidth = $event[0].size">
                ...
                <Pane ref="planEl" class="plan grab-bing position-relative" style="overflow: visible !important">
                  ...
                  <div class="position-absolute m-1 px-1 bottom-0 end-0 rounded bg-white d-flex" v-if="store.plan">
                  ...
                  <div class="position-absolute m-1 p-1 bottom-0 start-0 mb-1 rounded bg-white d-flex"
                    v-if="store.plan">
                    <div class="btn-group btn-group-xs">
                      <button class="btn btn-outline-secondary" :class="{
                        active:
                  ...
                  <svg width="100%" height="100%" :class="{ ready }">
                    <g :transform="transform">
                      ...
                      <AnimatedEdge v-for="(link, index) in rootLinks" :key="`${store.plan?.id}_link${index}`"
                        :d="lineGen(link)" :class="{
                          'never-executed': isNeverExecuted(link.target.data),
                        }" stroke-color="grey" :stroke-width="edgeWeight(
                          link.target.data[NodeProp.ACTUAL_ROWS_REVISED],
                        )
                          " :rows="link.target.data[NodeProp.ACTUAL_ROWS_REVISED]" />
                      <foreignObject v-for="(item, index) in rootDescendants" :key="`${store.plan?.id}_${index}`"
                        ...
                        <rect :x="getLayoutExtent(cte)[0] - padding / 4" :y="getLayoutExtent(cte)[2] - padding / 2"
                          :width="getLayoutExtent(cte)[1] -
                            ...
                            " :height="getLayoutExtent(cte)[3] - getLayoutExtent(cte)[2]
                              " stroke="#cfcfcf" stroke-width="2" fill="#cfcfcf" fill-opacity="10%" rx="5" ry="5">
                        </rect>
...
.mysql-tabs {
  ...
  .nav-link {
    ...
    &.active {
      color: var(--vscode-foreground);
      opacity: 1;
      background-color: var(--vscode-list-activeSelectionBackground, rgba(128, 128, 128, 0.15));
      box-shadow: inset 0 -2px 0 var(--vscode-button-background, #007acc);
      border-radius: 0;
      font-weight: 600;
    }
...
.mysql-header-btn {
  background-color: var(--vscode-button-background, rgba(128, 128, 128, 0.2));
  ...
  &:hover {
    background-color: var(--vscode-button-hoverBackground, rgba(128, 128, 128, 0.3));
    color: var(--vscode-button-foreground, white);
    opacity: 0.9;
  }
}
...
.header-divider {
  width: 1px;
  height: 24px;
  background-color: var(--vscode-widget-border, rgba(128, 128, 128, 0.3));
  margin: 0 0.25rem;
}
</style>
